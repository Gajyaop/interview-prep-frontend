...
export default function MockInterview() {
    ...
    const startInterview = async () => {
        setLoading(true);
        try {
            const res = await api.post('/api/ai/mock-interview/start', { topic, role, count: questionCount });
    ...
    const submitAnswer = async () => {
        ...
        try {
            const res = await api.post('/api/ai/mock-interview/evaluate', {
                question: questions[currentIndex], answer, topic,
            });
            setFeedback(res.data.feedback);
            setAllQA(prev => [...prev, { question: questions[currentIndex], answer, feedback: res.data.feedback }]);
        } catch (err) {
    ...
    const finishInterview = async () => {
        ...
        try {
            ...
            const res = await api.post('/api/ai/mock-interview/summary', { topic, role, questionsAndAnswers: transcript });
                    ...
                    <div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 mb-4">
                            <div className="flex items-start gap-3">
                                ...
                                <p className="text-gray-800 font-medium text-lg leading-relaxed">{questions[currentIndex]}</p>
                            </div>
                        </div>
                        ...
                        {feedback && (
                            <div className="bg-white rounded-2xl shadow-sm p-6 mb-4">
                                ...
                                <div className="bg-purple-50 rounded-xl p-4">
                                    <pre className="text-sm text-gray-700 whitespace-pre-wrap font-sans leading-relaxed">{feedback}</pre>
                                </div>
                            ...
                            {allQA.map((qa, i) => (
                                <div key={i} className="bg-white rounded-2xl shadow-sm p-6">
                                    <p className="font-semibold text-gray-800 mb-2">
                                        <span className="text-blue-600 mr-2">Q{i + 1}.</span>{qa.question}
                                    </p>
                                    ...
                                    <div className="bg-purple-50 rounded-lg p-3">
                                        <pre className="text-xs text-gray-600 whitespace-pre-wrap font-sans">{qa.feedback}</pre>
                                    </div>
                                </div>
                            ))}
                        </div>
